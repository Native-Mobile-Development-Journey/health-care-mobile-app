...
public class DoctorAppointmentsFragment extends Fragment implements AppointmentAdapter.OnAppointmentInteractionListener {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        RecyclerView recyclerView = view.findViewById(R.id.recycler_schedule_appointments);
    ...
    private void observeAppointments() {
        ...
        appointmentsListener = repository.observeDoctorAppointmentsFromFirestore(doctorId, new AppRepository.ListCallback<Appointment>() {
            @Override
    ...
    private void loadAppointments() {
        ...
        repository.getDoctorAppointments(doctorId, new AppRepository.ListCallback<Appointment>() {
            @Override
            public void onData(List<Appointment> items) {
                allAppointments.clear();
                allAppointments.addAll(items);
    ...
    private void applyFilters() {
        ...
        for (Appointment appointment : allAppointments) {
            ...
            if (!query.isEmpty() && !containsIgnoreCase(appointment.patientName, query)
    ...
    public void onAppointmentAction(Appointment appointment) {
        if (isAdded()) {
            Toast.makeText(requireContext(), "Action on " + appointment.normalizedStatus(), Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void showStatusChangeDialog(Appointment appointment) {
        androidx.appcompat.app.AlertDialog dialog = new androidx.appcompat.app.AlertDialog.Builder(requireContext())
                ...
                .setItems(new CharSequence[]{getString(R.string.status_completed), getString(R.string.status_cancel)}, (d, which) -> {
                    String newStatus = which == 0 ? Appointment.STATUS_COMPLETED : Appointment.STATUS_CANCELED;
